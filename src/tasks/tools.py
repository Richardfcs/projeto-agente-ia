...
class TemplateFillerTool(BaseTool):
    ...
    def _run(self, template_name: str, context: dict, owner_id: str, output_filename: str = None) -> str:
        """
        Preenche um template, salva o novo arquivo com um nome específico ou padrão,
        e retorna o ID do novo documento.
        """
        ...
        try:
            ...
            if not template_meta:
                return f"Erro: O template chamado '{template_name}' não foi encontrado."
            ...
            # --- LÓGICA DE NOMEAÇÃO INTELIGENTE ---
            if not output_filename:
                timestamp = datetime.utcnow().strftime('%Y%m%d')
                output_filename = f"Documento_de_{template_name.replace('.docx', '')}_{timestamp}.docx"
            
            output_file_id = fs.put(final_doc_stream, filename=output_filename)
